...
Usage:
  ...
  python configmap-reload.py -h

Feature request:
  - Создать объект log в который перенести все что связано с логами
  - Сделать проверку конфигурации после reload pgbouncer
"""
...
if __name__ == "__main__":
    ...
    p.add("-v", "--verbose",
          ...
          default=0)
    p.add("-c", "--config-path",
          help="Configuration path for watching",
          required=True,
          env_var='CONFIG_PATH')
    ...
    log.info("Initialization complete: %s" % (vars(args)))
